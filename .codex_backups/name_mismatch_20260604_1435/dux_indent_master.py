...
class DuxIndentMaster(Document):
    ...
    def set_delivery_stock_display(self):
        ...
        from dux_indent_master.api import update_indent_delivery_and_stock_fields

        update_indent_delivery_and_stock_fields(self)
    ...
    def on_submit(self):
        if self.status in ("Draft", "Pending", None, ""):
    ...
    def update_purchase_status(self):
        ...
        has_material_request = any(
            row.material_purchase_id and row.status != "Cancelled"
            for row in self.get("material_purchase") or []
        )
        ...
        elif has_material_request:
            self.status = "Material Request Raised"
        elif self.docstatus == 1:
            self.status = "Approved"
        elif self.status not in ("Draft", "Pending"):
            self.status = "Draft"
