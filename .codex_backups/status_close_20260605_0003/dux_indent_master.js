...
function get_selected_item_rows(frm) {
    ...
    const selected_names = frm.get_selected().items || [];
    return (frm.doc.items || []).filter((row) => selected_names.includes(row.name));
}
...
function bind_item_selection_events(frm) {
    ...
    grid.wrapper.off("change.dux_view_stock click.dux_view_stock", ".grid-row-check, .grid-check-all");
    grid.wrapper.on("change.dux_view_stock click.dux_view_stock", ".grid-row-check, .grid-check-all", () => {
        setTimeout(() => update_view_stock_button(frm), 100);
    });
}
